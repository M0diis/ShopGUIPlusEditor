...
public class Config {
    ...
    public ItemStack getAdjustButton(String type, ShopEditGUI shopEditGUI, ShopItem shopItem, int which) {
        ConfigurationSection sec = cfg.getConfigurationSection("edit-menu." + type + "-price-adjust-buttons.button-" + which);
        ...
        name = Utils.setPlaceholders(shopItem, shopEditGUI, name);
        ...
        for (int i = 0, loreSize = lore.size(); i < loreSize; i++) {
            ...
            String replaced = s.replaceAll("%amount%", String.valueOf(amount));

            replaced = Utils.format(replaced);

            replaced = Utils.setPlaceholders(shopItem, shopEditGUI, replaced);
    ...
    public void load(ShopGUIPlusEditor plugin) {
        ...
        Material buyPriceButtonMat = Material.getMaterial(
                cfg.getString("edit-menu.buy-price-button.material", "PAPER"));
        Material sellPriceButtonMat = Material.getMaterial(
                cfg.getString("edit-menu.sell-price-button.material", "PAPER"));

        Material fillItemMat = Material.getMaterial(
                cfg.getString("edit-menu.fill-item.material", "GREEN_STAINED_GLASS_PANE"));
